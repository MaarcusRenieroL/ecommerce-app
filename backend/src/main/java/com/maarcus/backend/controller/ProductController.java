...
public class ProductController {
	...
	@GetMapping("/get/{id}")
	public ResponseEntity<StandardResponse<Optional<Product>>> getProduct(@PathVariable String id) {
		try {
			return ResponseEntity.ok(new StandardResponse<>(HttpStatus.OK, "Product retrieved successfully", productService.getProduct(UUID.fromString(id))));
	...
	public ResponseEntity<StandardResponse<Product>> updateProduct(@PathVariable String id, @RequestBody Product product) {
		try {
			return ResponseEntity.ok(new StandardResponse<>(HttpStatus.OK, "Product updated successfully", productService.updateProduct(UUID.fromString(id), product)));
